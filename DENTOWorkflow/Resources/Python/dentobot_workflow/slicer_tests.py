...
class DENTOWorkflowTestMixin:
    ...
    def runTest(self) -> None:
        testNames = (
            ...
            "test_DENTOWorkflowCaseBundleRegistryNodeRecord",
            ...
            "test_DENTOWorkflowTrajectoryMetadataDoesNotInvalidateGeometry",
            "test_DENTOWorkflowInsertionMetadataDoesNotInvalidateTemplate",
            ...
            "test_DENTOWorkflowRestoredCaseJawGateDoesNotDeactivateImport",
            "test_DENTOWorkflowStep6SubstepNavigator",
    ...
    def test_DENTOWorkflowSegmentationReviewLogic(self) -> None:
        ...
        verificationWidget = DENTOWorkflowWidget.__new__(DENTOWorkflowWidget)
        verificationWidget.logic = logic
        verificationWidget.ui = type("VerificationUi", (), {})()
        verificationWidget.ui.trajectoryVerificationSmoothInterpolationCheckBox = (
            type("CheckBox", (), {"checked": True})()
        )
        verificationWidget._trajectoryVerificationPriorSliceState = {
            "segmentation2DRenderingMode": (
                logic.SEGMENTATION_2D_RENDERING_MODE_NATIVE
            )
        }
        verificationWidget._trajectoryVerificationEnabled = False
        verificationWidget._updatingTrajectoryVerificationUI = False
        verificationInputs = {
            "volumeNode": sourceVolume,
            "segmentationNode": displayQualityNode,
        }
        verificationWidget._trajectoryVerificationInputs = lambda: verificationInputs
        verificationWidget.onTrajectoryVerificationSmoothingToggled(True)
        self.assertTrue(logic.getScalarVolumeInterpolation(sourceVolume))
        self.assertEqual(
            logic.getSegmentation2DRenderingMode(displayQualityNode),
            logic.SEGMENTATION_2D_RENDERING_MODE_SMOOTH,
        ...
        self.assertEqual(
            displayQualityDisplayNode.GetDisplayRepresentationName2D(),
            logic.SEGMENTATION_CLOSED_SURFACE_REPRESENTATION,
        )
        verificationWidget.ui.trajectoryVerificationSmoothInterpolationCheckBox.checked = (
            False
        )
        verificationWidget.onTrajectoryVerificationSmoothingToggled(False)
        self.assertFalse(logic.getScalarVolumeInterpolation(sourceVolume))
        self.assertEqual(
            logic.getSegmentation2DRenderingMode(displayQualityNode),
            logic.SEGMENTATION_2D_RENDERING_MODE_NATIVE,
    ...
    def test_DENTOWorkflowCaseBundleRegistryNodeRecord(self) -> None:
        ...
        expected = {
            ...
            logic.REGISTRY_TRAJECTORY_SLOT_ATTRIBUTE: "1",
            logic.REGISTRY_TRAJECTORY_FINGERPRINT_ATTRIBUTE: "trajectory-fingerprint",
            logic.REGISTRY_GUIDE_SET_ID_ATTRIBUTE: "FDI31-guide",
            logic.REGISTRY_TEMPLATE_ID_ATTRIBUTE: "FDI31-template",
            logic.REGISTRY_SHELL_ID_ATTRIBUTE: "FDI31-shell",
        }
        for name, value in expected.items():
            trajectory.SetAttribute(name, value)
    ...
    def test_DENTOWorkflowAssistedRootTrajectoryGeneration(self) -> None:
        ...
        singleLines, singleAnalysis = logic.generateAssistedTrajectories(
            singleEntry, segmentationNode, "tooth-16", 1, targetBoundsRoi,
        )
        self.assertEqual(
            singleAnalysis["endpointMethod"],
            "FirstSharedPulpIntersectionV2",
        )
        pulpSurface = logic._getClosedSurfaceCopy(segmentationNode, "pulp-16")
        pulpDistance = vtk.vtkImplicitPolyDataDistance()
        pulpDistance.SetInput(pulpSurface)
        self.assertLessEqual(
            pulpDistance.EvaluateFunction(
                logic.getTrajectorySummary(singleLines[0])["targetRas"]
            ),
            1e-4,
        )
        ...
        self.assertEqual(len(analysis["binaryMaskBoundaryTargetsRas"]), 2)
        self.assertEqual(len(analysis["displaySurfaceBoundaryTargetsRas"]), 2)
        self.assertEqual(len(analysis["surfaceOffsetsMm"]), 2)
        ...
        for index, trajectoryNode in enumerate(trajectories):
            ...
            self.assertLessEqual(
                pulpDistance.EvaluateFunction(summary["targetRas"]), 1e-4
            )
            ...
            binaryFraction = (
                analysis["binaryMaskBoundaryTargetsRas"][index][2] - entry[2]
            ) / (originalTarget[2] - entry[2])
            surfaceFraction = (
                analysis["displaySurfaceBoundaryTargetsRas"][index][2] - entry[2]
            ) / (originalTarget[2] - entry[2])
            self.assertAlmostEqual(
                fraction, max(binaryFraction, surfaceFraction), places=5
            )
    ...
    def test_DENTOWorkflowCaseFoundation(self) -> None:
        ...
        volume.SetOrigin(-100.0, -150.0, -100.0)
        parameterNode.inputVolume = volume
        segmentation = slicer.mrmlScene.AddNewNodeByClass(
            "vtkMRMLSegmentationNode",
            "Step6CaseJawMasks",
        ...
        addCubeSegment(
            ...
            (-2.0, 2.0, -92.0, -88.0, -12.0, -8.0),
        )
        addCubeSegment(
            "lower-tooth-31",
            "lower_left_central_incisor_fdi31",
            (-2.0, 2.0, -92.0, -88.0, -14.0, -10.0),
        )
        addCubeSegment(
            "maxilla",
            "maxilla",
        ...
        self.assertLess(
            anatomyReview["condylarReview"]["leftRasXmm"],
            anatomyReview["condylarReview"]["rightRasXmm"],
        )
        ...
        self.assertTrue(
            logic.isStep6DerivedAnatomyNode(
                ...
                logic.STEP6_FIXED_UPPER_ANATOMY_ROLE,
            )
        )
        self.assertTrue(
            logic.isStep6DerivedAnatomyNode(
                parameterNode.step6MovingLowerAnatomy,
                logic.STEP6_MOVING_LOWER_ANATOMY_ROLE,
            )
        )
        self.assertIs(
            parameterNode.step6MovingLowerAnatomy.GetParentTransformNode(),
            transform,
        )
    ...
    def test_DENTOWorkflowStep6SubstepNavigator(self) -> None:
        ...
        widget = slicer.modules.dentoworkflow.widgetRepresentation().self()
        widget._applyDENTOBOTGuiMode(GUI_MODE_LEGACY, persist=False)
        widget.initializeParameterNode()
        widget._setWorkflowStage(10, ensureVisible=False)
        parameterNode = widget._parameterNode
        importedBefore = bool(parameterNode.step6PlanningContextImported)
        rosNodeCountBefore = len(
            slicer.util.getNodesByClass("vtkMRMLROS2RobotNode")
        )

        self.assertTrue(widget._step6SubstepNavigator.visible)
        self.assertEqual(widget._step6SubstepComboBox.count, 7)
        self.assertEqual(widget._step6SubstepComboBox.currentIndex, 0)
        self.assertTrue(widget.ui.step6PlanningContextGroupBox.visible)
        self.assertFalse(widget.ui.step6MountLockGroupBox.visible)
        self.assertFalse(widget._robotSimulationPanel.visualizationGroup.visible)
        self.assertFalse(widget._step6PreviousSubstepButton.enabled)
        self.assertTrue(widget._step6NextSubstepButton.enabled)

        widget._step6SubstepComboBox.currentIndex = 1
        slicer.app.processEvents()
        self.assertFalse(widget.ui.step6PlanningContextGroupBox.visible)
        self.assertTrue(widget._robotSimulationPanel.visualizationGroup.visible)
        self.assertTrue(widget.ui.step6MountLockGroupBox.visible)
        self.assertFalse(widget.ui.step6TaskJointLimitsGroupBox.visible)

        widget._onNextStep6Substep()
        self.assertEqual(widget._step6SubstepComboBox.currentIndex, 2)
        self.assertTrue(widget.ui.step6TaskJointLimitsGroupBox.visible)
        self.assertTrue(widget._robotSimulationPanel.homeGroup.visible)
        self.assertFalse(widget.ui.step6MountLockGroupBox.visible)

        widget._onApplicationShellSubstepSelected("robot_simulation", 4)
        self.assertTrue(widget._robotSimulationPanel.runtimeGroup.visible)
        self.assertTrue(widget._robotSimulationPanel.collisionGroup.visible)
        self.assertFalse(widget._robotSimulationPanel.approachGroup.visible)
        widget._onApplicationShellSubstepSelected("robot_simulation", 6)
        self.assertTrue(widget._robotSimulationPanel.drillingGroup.visible)
        self.assertFalse(widget._step6NextSubstepButton.enabled)
        ...
        self.assertEqual(
            len(slicer.util.getNodesByClass("vtkMRMLROS2RobotNode")),
            rosNodeCountBefore,
        )
        self.delayDisplay("DENTOWorkflow Step 6 substep navigator test passed")
    ...
    def test_DENTOWorkflowTrajectoryMetadataDoesNotInvalidateGeometry(self) -> None:
        """Only Entry/Target state changes may invalidate descendants."""

        widget = slicer.modules.dentoworkflow.widgetRepresentation().self()
        widget.setParameterNode(None)
        logic = widget.logic
        parameterNode = logic.getParameterNode()
        trajectory = logic.createTrajectoryNode("GeometryEventTrajectory")
        trajectory.AddControlPointWorld(vtk.vtkVector3d(1.0, 2.0, 3.0))
        trajectory.AddControlPointWorld(vtk.vtkVector3d(4.0, 5.0, 6.0))
        parameterNode.trajectoryLine = trajectory
        widget.setParameterNode(parameterNode)
        widget._bindPlanningTrajectoryNode(trajectory)

        invalidations = []
        originalInvalidate = logic.invalidateStep6TaskConfirmation
        logic.invalidateStep6TaskConfirmation = (
            lambda _parameterNode, reason: invalidations.append(str(reason))
        )
        try:
            baseline = widget._trajectoryGeometrySnapshot(trajectory)
            trajectory.SetAttribute("DENTOBOT.TestPresentationMetadata", "one")
            trajectory.SetSelectable(not trajectory.GetSelectable())
            slicer.app.processEvents()
            self.assertEqual(invalidations, [])
            self.assertTrue(
                widget._trajectoryGeometrySnapshotsMatch(
                    baseline,
                    widget._trajectoryGeometrySnapshot(trajectory),
                )
            )

            trajectory.SetNthControlPointPositionWorld(
                1,
                vtk.vtkVector3d(4.0, 5.0, 6.01),
            )
            slicer.app.processEvents()
            self.assertEqual(len(invalidations), 1)
            self.assertIn("trajectory geometry changed", invalidations[0])
        finally:
            logic.invalidateStep6TaskConfirmation = originalInvalidate
            widget.setParameterNode(None)

        self.delayDisplay(
            "DENTOWorkflow trajectory geometry-event discrimination passed"
        )

    def test_DENTOWorkflowInsertionMetadataDoesNotInvalidateTemplate(self) -> None:
        """Only Approach/Seat geometry changes may stale Step 5B/5C."""

        widget = slicer.modules.dentoworkflow.widgetRepresentation().self()
        widget.setParameterNode(None)
        logic = widget.logic
        parameterNode = logic.getParameterNode()
        insertion = slicer.mrmlScene.AddNewNodeByClass(
            "vtkMRMLMarkupsLineNode",
            "InsertionGeometryEvent",
        )
        insertion.SetAttribute(
            "DENTOBOT.MarkupsRole",
            "TemplateInsertionDirection",
        )
        insertion.AddControlPointWorld(vtk.vtkVector3d(1.0, 2.0, 3.0))
        insertion.AddControlPointWorld(vtk.vtkVector3d(1.0, 2.0, 9.0))
        insertion.SetLocked(True)
        insertion.SetSelectable(False)
        parameterNode.templateInsertionDirection = insertion
        widget.setParameterNode(parameterNode)
        widget._bindTemplateInsertionDirectionNode(insertion)

        invalidations = []
        originalInvalidate = widget._invalidateTemplateUndercutDownstream
        widget._invalidateTemplateUndercutDownstream = (
            lambda reason: invalidations.append(str(reason)) or True
        )
        try:
            baseline = widget._trajectoryGeometrySnapshot(insertion)
            insertion.SetAttribute("DENTOBOT.TestPresentationMetadata", "one")
            insertion.SetLocked(False)
            insertion.SetSelectable(True)
            insertion.SetNthControlPointLabel(0, "Approach")
            slicer.app.processEvents()
            self.assertEqual(invalidations, [])
            self.assertTrue(
                widget._trajectoryGeometrySnapshotsMatch(
                    baseline,
                    widget._trajectoryGeometrySnapshot(insertion),
                )
            )

            insertion.SetNthControlPointPositionWorld(
                1,
                vtk.vtkVector3d(1.0, 2.0, 9.01),
            )
            slicer.app.processEvents()
            self.assertEqual(len(invalidations), 1)
            self.assertIn("insertion direction changed", invalidations[0])
        finally:
            widget._invalidateTemplateUndercutDownstream = originalInvalidate
            widget.setParameterNode(None)

        self.delayDisplay(
            "DENTOWorkflow insertion geometry-event discrimination passed"
        )
    ...
    def test_DENTOWorkflowVisibleTemplateSupportSurface(self) -> None:
        ...
        focusedPreparedBranch = bool(
            getattr(self, "_focusedPreparedBranchSmoke", False)
        )
        parameterNode = logic.getParameterNode()
        ...
        for segmentId, centerX in (() if focusedPreparedBranch else (("tooth-16", -2.25),)):
        ...
        parameterNode.teethSegmentation = segmentationNode
        parameterNode.targetToothSegmentId = "tooth-16"
        parameterNode.trajectoryLine = directionTrajectory
        ...
        self.assertEqual(
            finalDetails["assembly"]["trajectoryCount"], len(guideTrajectories)
        )
        ...
        self.assertEqual(
            finalDetails["assembly"]["shellContactBranches"]["method"],
            "FourIndependentBoreTangentShellAttachmentsV2",
        )
        self.assertGreaterEqual(
            finalDetails["assembly"]["shellContactBranches"][
                "minimumBoreSurfaceClearanceMm"
            ],
            0.3,
        )
        self.assertGreaterEqual(
            finalDetails["assembly"]["shellContactBranches"][
                "minimumReinforcedAnnularLigamentMm"
            ],
            0.3,
        )
        for branch in finalDetails["assembly"]["shellContactBranches"][
            "branches"
        ]:
            self.assertEqual(
                branch["method"],
                "BoreTangentOverlappingCylindricalReinforcementV2",
            )
            self.assertAlmostEqual(
                branch["boreCenterlineClearanceMm"]
                - branch["bridgeDiameterMm"] / 2.0
                - branch["boreRadiusMm"],
                0.3,
                places=6,
            )
        boreProtection = finalDetails["assembly"]["targetDocking"][
            "boreProtection"
        ]
        self.assertEqual(boreProtection["channelCount"], 4)
        self.assertGreaterEqual(boreProtection["endOverhangMm"], 2.3)
        self.assertEqual(
        ...
        self.assertGreater(
            finalDetails["fusion"]["channelExcludedOccupiedSampleCount"],
            0,
        )
        self.assertEqual(
            finalDetails["fusion"]["channelResidualOccupiedSampleCount"],
            0,
        )
        finalDistance = vtk.vtkImplicitPolyDataDistance()
        finalDistance.SetInput(finalTemplate.GetPolyData())
        for dock in targetDockingDetails["metrics"]["docks"]:
            top = np.asarray(dock["topFaceCenterRas"], dtype=float)
            terminal = np.asarray(dock["terminalCenterRas"], dtype=float)
            axis = np.asarray(dock["axisRas"], dtype=float)
            for sample in (
                top - axis,
                (top + terminal) / 2.0,
                terminal + 0.5 * axis,
            ):
                self.assertGreater(finalDistance.EvaluateFunction(sample), 0.0)
        ...
        currentFinalSchema = finalTemplate.GetAttribute(
            "DENTOBOT.FinalGuideSchemaVersion"
        )
        self.assertEqual(currentFinalSchema, logic.TEMPLATE_FINAL_GUIDE_SCHEMA_VERSION)
        finalTemplate.SetAttribute("DENTOBOT.FinalGuideSchemaVersion", "1.0")
        self.assertEqual(
            logic.getFinalPrintableTemplateSummary(finalTemplate)["geometryState"],
            "Stale",
        )
        finalTemplate.SetAttribute(
            "DENTOBOT.FinalGuideSchemaVersion", currentFinalSchema
        )

        if focusedPreparedBranch:
            ...
            selectedBranchId = registry["selected_branch_id"]
            self.assertTrue(selectedBranchId)
            eligibility = logic.evaluatePreparedBranchEligibility(parameterNode)
            self.assertEqual(eligibility["reason"], "VALID")
            activeBefore = (
            ...
            with self.assertRaisesRegex(ValueError, "verified PreparedBranch"):
                logic.activateDentoCasePreparedBranch(parameterNode, "missing-branch")
            self.assertEqual(
                activeBefore,
                (
                    parameterNode.trajectoryLine,
            ...
            report = logic.importStep6PlanningContext(parameterNode)
            self.assertTrue(report.ready)
            self.assertTrue(parameterNode.step6PlanningContextImported)
            self.assertEqual(
                [node.GetID() for node in logic.getSelectedTemplateGuideTrajectories()],
                [node.GetID() for node in guideTrajectories],
            )
            self.assertEqual(
                parse_trajectory_registry(parameterNode.step6TrajectoryRegistryJson)[
                    "selected_branch_id"
                ],
                selectedBranchId,
            )
