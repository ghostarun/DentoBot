...
PC roll-out remain open. Native Windows Slicer is retained only as a legacy
Steps 0–5 fallback (`PLAT-U-02`) with Step 6 and SlicerROS2 disabled.

| Host profile | Slicer | External inference | Docker | ROS/SlicerROS2 |
|---|---|---|---|---|
| Windows 11 + WSLg (primary) | Linux Slicer 5.10 in `dentobot-slicerros2` (WSLg) | Direct Linux Python in WSL (`cpu` or `cuda:0`) | Required | Same Ubuntu container stack; Steps 0–6 simulation |
...
is private, so Docker must authenticate to GHCR before installation. One
