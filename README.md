...
stack as Ubuntu. It requires Windows 11 with WSLg, a WSL Ubuntu distribution,
and exactly one Docker daemon: Docker Engine inside that distribution by
default, or Docker Desktop with WSL integration as an alternative. Do not run
both daemon paths together. NVIDIA container support is required when using
`cuda:0`. WSLg is accepted for functional GUI checks, not rendering performance
acceptance. See [Workspace/docs/WINDOWS_SETUP.md](Workspace/docs/WINDOWS_SETUP.md)
for both host bootstraps. Lab users install immutable dated tags; `main` is not
a lab release.
...
4. Build or pull the Compose image
   `dentobot/slicerros2:jazzy-moveit-sim-20260909` (see
